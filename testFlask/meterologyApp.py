...
def preprocess():
    ...
    brokers =BrokerNetwork.all()
    if len(pubs) > 0 and len(brokers) > 0:
        print("adding the brokers and pubs")
        pushDefaultPubs()
        pushDefaultBrokers
...
# @app.route("/api/elevationPoint")
# def getEle():
#     return elevationPoint.elevation()


# @app.route("/api/addPubs")
# def addPubs():
#     response = insertPub()
#     return response
...
# def insertPub():
#     db.pubs.insert_one({'name':'elevation','url':'https://api.stormglass.io/v2/elevation/point'})
#     return jsonify(message="success")

# @app.route("/api/getPubs")
# def getPubs():
#     pubs = db.pubs.find()
#     return jsonify([pub for pub in pubs])
...
def pushDefaultBrokers():
    broker01 = BrokerNetwork(broker_name="middleware",
                            pub_name="elevationPoint",brokerAddress="http://localhost:5000/")
    broker01.save()
    broker02 = BrokerNetwork(broker_name="middleware",
                            pub_name="astronomy",brokerAddress="http://localhost:5000/")
    broker02.save()
    broker03 = BrokerNetwork(broker_name="middleware01",
                             pub_name = "bio",brokerAddress="http://localhost:5001/")
    broker03.save()
    broker04 = BrokerNetwork(broker_name = "middleware02",
                            pub_name = "seaLevel",brokerAddress="http://localhost:5002/")
    broker04.save()
...
@app.route("/api/subscribedBrokerInfo",methods=['GET'])
def getSubscribedBrokerInfo():
    username = request.args.get('username')
    print("username",username)
    pubsub = PubSub.objects(sub_username=username[1:]).all()
    brokerNet = BrokerNetwork.objects().all()
    topics = []
    matchedBroker = []
    for pub in pubsub:
        topics.append(pub['pub_name'])
    for broker in brokerNet:
        if broker['pub_name'] in topics:
            matchedBroker.append({"broker_name": broker["broker_name"],"pub_name":broker["pub_name"],"brokerAddress":broker['brokerAddress']})
    print("Earth")
    print(matchedBroker)
    return json.dumps(matchedBroker)
...
def addME():
    ...
    "brokerAddress": "http://localhost:5000/"
    },
    {
    ...
    res0 = requests.post('http://middleware01:5001/api/addBroker',json.dumps(myRecord))
    if not res0:
        return "Failed"
    else:
